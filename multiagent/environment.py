...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        self.discrete_action_input = discrete_action #:True
        ...
        for agent in self.agents:
            ...
            # observation space obs_dim=10
            obs_dim = len(observation_callback(agent, self.world))
            self.observation_space.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(obs_dim,), dtype=np.float32))
            agent.action.c = np.zeros(self.world.dim_c)

        # rendering
        self.cam_range = cam_range + 0.5
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.action_space[i]) #，action_n[i]
        # advance world state
        self.world.step()
        for agent in self.agents:
        ...
        # all agents get total reward in cooperative case share reward.
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        agent.action.c = np.zeros(self.world.dim_c)
        # process action
        if isinstance(action_space, MultiDiscrete): #False
            act = []
        ...
        if agent.movable:
            # physical action
            if self.discrete_action_input: #True
                agent.action.u = np.zeros(self.world.dim_p)
                ...
                if self.discrete_action_space:
                    ...
                    agent.action.u = action[0]*np.array([10., 10.]) + np.array([-5., -5.])
            sensitivity = 1.0
            if agent.accel is not None:# Fasle
    ...
    def render(self, mode='human', attn=None):
        ...
        if self.render_geoms is None or True:
            ...
            for entity in self.world.entities:
                geom = rendering.make_circle(entity.size/2)
                xform = rendering.Transform()
                if 'agent' in entity.name:
                    # if entity.live ==1:
                    # geom.set_color(*entity.color, alpha=0.5)
                    # geom.set_color(*np.array([46/255,139/255,87/255]), alpha=0.5)
                    geom.set_color(*np.array([0/255,255/255,0/255]), alpha=0.5)

                    self.render_texts.append(entity.name[5:]) #
                    self.render_texts_xforms.append(entity.state.p_pos)#
                    # else:
                    #     geom.set_color(*entity.color, alpha=0.0)
                else:
                    geom.set_color(*entity.color)
                    self.render_texts.append(entity.name[8:])#
                    self.render_texts_xforms.append(entity.state.p_pos)#
            ...
            self.render_geoms_xform_obs = []
            for entity in self.world.policy_agents:
                geom = rendering.make_circle(0.5, 50, True)
                xform = rendering.Transform()
                if 'agent' in entity.name:
                    # geom.set_color(*entity.color, alpha=0.5)
                    geom.set_color(*np.array([0/255,191/255,255/255]), alpha=0.3)

                else:
                    geom.set_color(*entity.color)

                geom.add_attr(xform)
                self.render_geoms_obs.append(geom)
                self.render_geoms_xform_obs.append(xform)

            self.render_geoms_obs_critical = []
            self.render_geoms_xform_obs_critical = []
            # for entity in self.world.policy_agents:
            #     geom = rendering.make_circle(0.5, 30, False)
            #     xform = rendering.Transform()
            #     if 'agent' in entity.name:
            #         geom.set_color(*np.array([0.0, 0.0, 0.5]), alpha=0.5)
            #         # geom.set_color(*entity.color, alpha=0.5)
            #
            #     else:
            #         geom.set_color(*entity.color)
            #
            #         # geom.set_color(*np.array([0.5, 0.0, 0.0]))
            #
            #     geom.add_attr(xform)
            #     self.render_geoms_obs_critical.append(geom)
            #     self.render_geoms_xform_obs_critical.append(xform)
            ...
            for viewer in self.viewers:
                ...
                for geom in self.render_geoms:
                    viewer.add_geom(geom)
                for geom in self.render_geoms_obs_critical:
                    viewer.add_geom(geom)
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.policy_agents):
                self.render_geoms_xform_obs[e].set_translation(*entity.state.p_pos)
            for e, entity in enumerate(self.world.entities):
                self.render_geoms_xform[e].set_translation(*entity.state.p_pos)
            # for e, entity in enumerate(self.world.policy_agents):
            #     self.render_geoms_xform_obs_critical[e].set_translation(*entity.state.p_pos)
