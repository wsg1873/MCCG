...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space, recurrent_hidden_state_size):
        ...
        self.obs_1 = torch.zeros(num_steps + 1, num_processes, *obs_shape)
        self.obs_2 = torch.zeros(num_steps + 1, num_processes, *obs_shape)
        self.obs_3 = torch.zeros(num_steps + 1, num_processes, *obs_shape)
        self.obs_4 = torch.zeros(num_steps + 1, num_processes, *obs_shape)
    ...
    def to(self, device):
        ...
        self.obs_3 = self.obs_3.to(device)
        self.obs_4 = self.obs_4.to(device)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs, value_preds, rewards, masks):
        ...
        self.obs_1[self.step + 1].copy_(self.obs[self.step]*masks)
        self.obs_2[self.step + 1].copy_(self.obs_1[self.step]*masks)
        self.obs_3[self.step + 1].copy_(self.obs_2[self.step]*masks)
        self.obs_4[self.step + 1].copy_(self.obs_3[self.step]*masks)
    ...
    def after_update(self):
        ...
        self.obs_1[0].copy_(self.obs_1[-1])
        self.obs_2[0].copy_(self.obs_2[-1])
        self.obs_3[0].copy_(self.obs_3[-1])
        self.obs_4[0].copy_(self.obs_4[-1])
