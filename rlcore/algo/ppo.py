...
class JointPPO():
    ...
    def update(self, rollouts_list):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                obs_batch, obs_1_batch, obs_2_batch, obs_3_batch, obs_4_batch,recurrent_hidden_states_batch, actions_batch, value_preds_batch, return_batch,\
                masks_batch, old_action_log_probs_batch, adv_targ = sample
                # Reshape to do in a single forward pass for all steps
                values, action_log_probs, dist_entropy, _ = self.actor_critic.evaluate_actions(obs_batch, obs_1_batch, obs_2_batch, obs_3_batch, obs_4_batch,
                                 recurrent_hidden_states_batch, masks_batch, actions_batch)
...
def magent_feed_forward_generator(rollouts_list, advantages_list, num_mini_batch):
    ...
    for indices in sampler:
        ...
        obs_1_batch=torch.cat([rollout.obs_1[:-1].view(-1,*rollout.obs_1.size()[2:])[indices] for rollout in rollouts_list],0)
        obs_2_batch=torch.cat([rollout.obs_2[:-1].view(-1,*rollout.obs_2.size()[2:])[indices] for rollout in rollouts_list],0)
        obs_3_batch=torch.cat([rollout.obs_3[:-1].view(-1,*rollout.obs_3.size()[2:])[indices] for rollout in rollouts_list],0)
        obs_4_batch=torch.cat([rollout.obs_4[:-1].view(-1,*rollout.obs_4.size()[2:])[indices] for rollout in rollouts_list],0)
        ...
        yield obs_batch, obs_1_batch, obs_2_batch, obs_3_batch, obs_4_batch, recurrent_hidden_states_batch, actions_batch, value_preds_batch, return_batch,\
